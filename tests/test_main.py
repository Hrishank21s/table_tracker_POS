...
def test_start_session_rejects_already_active_table(client, db):
    start_session(client, table_id=1)
    response = client.post("/sessions/start", json={"table_id": 1})
    assert response.status_code == 409
    assert response.json()["detail"] == "Table already has an active session"
    assert db.execute("SELECT COUNT(*) FROM sessions").fetchone()[0] == 1


def test_start_session_unknown_customer(client, db):
    response = client.post("/sessions/start", json={"table_id": 1, "customer_id": 999})
    assert response.status_code == 404
...
def test_add_item_rejects_closed_session(client, db):
    session_id = start_session(client)
    client.post(
        "/sessions/checkout",
        json={"session_id": session_id, "payment_status": "PAID", "total_minutes": 5},
    )

    response = client.post("/sessions/add_item", json={"session_id": session_id, "item_id": 1})
    assert response.status_code == 409
    assert response.json()["detail"] == "Session is already closed"
    assert db.execute("SELECT COUNT(*) FROM session_items").fetchone()[0] == 0


def test_checkout_rejects_second_checkout(client, db, customer):
    session_id = start_session(client, customer_id=customer)
    client.post(
        "/sessions/checkout",
        json={"session_id": session_id, "payment_status": "UNPAID", "total_minutes": 10},
    )
...
def test_checkout_rejects_unknown_payment_status(client, db):
    session_id = start_session(client)
    response = client.post(
        "/sessions/checkout",
        json={"session_id": session_id, "payment_status": "REFUND", "total_minutes": 5},
    )
...
def test_checkout_rejects_negative_minutes(client):
    session_id = start_session(client)
    response = client.post(
        "/sessions/checkout",
        json={"session_id": session_id, "payment_status": "PAID", "total_minutes": -5},
...
def test_database_failure_returns_503(tmp_path, monkeypatch):
    monkeypatch.setattr(main, "DB_PATH", str(tmp_path / "missing.db"))
    response = TestClient(main.app).get("/menu")
